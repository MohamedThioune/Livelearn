                ...
                <form action="" method="POST" class="wrapperClone" id="attributes">
                    ...
                    if(!empty($data_en)){
                        ...
                        <div class="input-group-course">
                            <label for="">Dates betweene</label>
                            <input type="text" name="between_date[]"  id="date_details" value="<?= $data_between ?>" class="datepicker Txt_Date" placeholder="Pick the multiple dates" style="cursor: pointer;">
                            <ul id="new_date" class="new_date"></ul>
                            <div class="groupBtn-date">
                                <div class="contentInputRemovedate w-100" style="display:none;" id="removedate">
                                    <input type="button" name="remove_date" id="remove_date" value="-Verwijderen" class="btn-remove-date">
                                </div>
                                <div class="w-100">
                                    <input type="button" name="add_date" id="add_date" value="+More" class="btn-more-date">
                                </div>
                            </div>
                        </div>
                        <div class="groupInputDate blockDateW100">
                    ...
                    ?>
                        <div id="dateBlock">
                            ...
                            <div class="input-group-course">
                                ...
                                <input type="date" name="between_date[]" id="date_details" class="" placeholder="" style="cursor: pointer;">
                                <ul id="new_date" class="new_date"></ul>
                                <div class="groupBtn-date">
                                    <div class="contentInputRemovedate w-100" style="display:none;" id="removedate">
                                        <input type="button" name="remove_date" id="remove_date" value="-Verwijderen" class="btn-remove-date">
                        ...
                        <div class="new_blockDateCloned"></div>
                    ...
                    <div class="buttons groupBtnData">
                        <button type="button" id="add_block-date" class=""> +Extra startdatum</button>
                    ...
                    <div class="blockForClone">
                            <div class="attr">
                                ...
                                <div class="input-group-course">
                                    <label for="">Dates between</label>
                                    <input type="text" name="between_date[]"  id="date_details" class="datepicker Txt_Date" placeholder="Pick the multiple dates" style="cursor: pointer;">
                                    <ul id="new_date" class="new_date"></ul>
                                </div>
                                <div class="groupInputDate blockDateW100">
                                    <div class="input-group">
...
<script>
    jQuery(function ($) {
        $('#add_date').click(function () {
            var button = $('#date_details').clone();
            button.find('input').val('');
            button.removeAttr('id');//ID of an element must be uniue
            $(".new_date").append(button);
            $('#removedate').show();
        });
        $('#remove_date').click(function (e) {
            $("#new_date > input").last().remove();
            $('#removedate').toggle( !$("#new_date").is(":empty") );
            e.preventDefault();
        });
    });
...
<script>
    jQuery(function ($) {
        $('#add_block-date').click(function () {
            var button = $('#dateBlock').clone(true).insertAfter($('#add_date'));
            button.find('remove_date').val('');
            button.removeAttr('id');//ID of an element must be uniue
            $(".new_blockDateCloned").append(button);
            $('#removedate').show();
        });
        $('#remove_block-date').click(function (e) {
            $(".new_blockDateCloned > .dateBlock").last().remove();
            $('#removedate').toggle( !$("#new_date").is(":empty") );
            e.preventDefault();
        });
    });
