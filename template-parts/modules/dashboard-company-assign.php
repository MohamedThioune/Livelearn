...
//$entries_from_payment = $wpdb->get_results( "SELECT `entry_id` FROM wpe7_gf_entry_meta WHERE form_id = 1 AND meta_value =" . $_GET['payment_id']);//meta value 10
//$entries = array();
//foreach($entries_from_payment as $entry_for_payment){
//    array_push($entries,$entry_for_payment->entry_id);
//}

//
//$data = array();
//foreach($entries as $entry){
//    $check_course_id = $wpdb->get_results( "SELECT `entry_id`,`meta_value` FROM wpe7_gf_entry_meta WHERE form_id = 1 AND entry_id = ".$entry." AND meta_key = '9' AND meta_value = " . $_GET['course_id']."");//compare
//
//
//    if($check_course_id){
//        foreach($check_course_id as $subscription_entries){
//            $results = $wpdb->get_results('SELECT meta_key, meta_value FROM wpe7_gf_entry_meta WHERE entry_id = '.$subscription_entries->entry_id);
//            $subscribers = array();
//
//            foreach($results as $subscription){
//                if($subscription->meta_key == 2){
//                    $subscribers['voornaam'] = $subscription->meta_value;
//                }
//                if($subscription->meta_key == 3){
//                    $subscribers['achternaam'] = $subscription->meta_value;
//                }
//                if($subscription->meta_key == 5){
//                    $subscribers['email'] = $subscription->meta_value;
//                }
//                if($subscription->meta_key == 11){
//                    $subscribers['datenr'] = $subscription->meta_value;
//                }
//            }
