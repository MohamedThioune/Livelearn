...
function xmlParse($data)
{
    ...
    $website = $website_urls[$data];
    //Get the URL content
    $file = get_stylesheet_directory_uri() . "/" . $website;
    $xml = simplexml_load_file($file);
    $datas_xml = $xml->program;

    $author_id = null;

    echo "<h3>" . $datas_xml[0]->programClassification->orgUnitId . " running <i class='fas fa-spinner fa-pulse'></i></h3><br><br>";

    //Retrieve courses
    // $i = 0;
    foreach ($datas_xml as $key => $datum) {
        // $i++;
        // if($i == 2)
        //   break;
        ...
        if (!empty($datum->programSchedule->programRun)) {
            foreach ($datum->programSchedule->programRun as $program) {
                ...
                array_push($datas_locaties_xml, $infos);
            }

            $datas_locaties = join('~', $datas_locaties_xml);
        }
        ...
        if (!isset($check_image[0]) && !isset($check_title[0])) {
            ...
            if (empty($course)) {
                ...
                if ($post['type'] != $course->type) {
                    ...
                    $where = ['id' => $course->id];
                    $updated = $wpdb->update($table, $datas, $where);
