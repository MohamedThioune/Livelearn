...
{
    $subject = "LIVELEARN Reset your password";
    $headers = array( 'Content-Type: text/html; charset=UTF-8','From: Livelearn <info@livelearn.nl>' );
    $mail_became_manager_body = '
    <!DOCTYPE html>
<html
...
>
  <head>
    <title><firs-name>, je hebt een nieuwe rol</firs-name></title>
    <!--[if !mso]><!-->
                        ...
                        >
                          ...
                          >
                            <tbody>
                              <tr>
                                <td style="width: 50px">
                                  <a href="http://livelearn.nl" target="_blank"
                                    ><img
                                      alt=""
                                      height="auto"
                                      src="https://0gt5q.mjt.lu/tplimg/0gt5q/b/lurx1/l0xh.png"
                                      style="
                                        border: none;
                                        display: block;
                                        outline: none;
                            ...
                            >
                              <span
                                style="
                                  ...
                                  line-height: 35px;
                                "
                                ><b>Resetting your password</b></span
                              >
                            </h1>
                          </div>
                        </td>
                            ...
                            >
                              <span
                                style="
                                  ...
                                  line-height: 22px;
                                "
                                >Your account is being requested to be reset. If
                                it is not yours please ignore this email !!!
                                &nbsp;</span
                              ><br />&nbsp;
                            </p>
                            <p class="text-build-content" data-testid="S_MPaSnC0uI" style="margin: 10px 0">'.$url.'</p>
                            <p
                              class="text-build-content"
                              data-testid="S_MPaSnC0uI"
                              style="margin: 10px 0"
                              ...
                              <tr>
                                <td
                                  ...
                                  style="
                                    ...
                                    border-radius: 5px;
                                    cursor: auto;
                                    mso-padding-alt: 10px 25px 10px 25px;
                                    background: #023356;
                                  "
                                  valign="middle"
                                >
    ...
    return new WP_REST_Response(
        array(
            ...
            'count_course_type'=>[
               ...
               'Opleidingen'=> $numTypos['course'],
               'Artikel' => $numTypos['article'],
               'Masterclass' => $numTypos['masterclass'],
               'Workshop' => $numTypos['workshop'],
               'E_Learning' => $numTypos['elearning'],
...
function reset_password_email($data)
{
    $email = $data['email'];
    ...
    if (!$user) {
        return new WP_Error('user_not_found', 'this email is not matching for any user', array('status' => 404));
    }
        // generate token
        $reset_token = bin2hex(random_bytes(32));
        $reset_token_expiry = time() + 3600; // expirate in 1 hour

        update_field('token_reset_password',['token'=>$reset_token,'expire_at'=>$reset_token_expiry],'user_'.$user->ID);
        // send email
    $url = "https://livelearn.nl/new-password?token=$reset_token&email=$email";
    $name = $user->data->display_name ?? $user->data->user_email;
    // send the email
    sendEmailResetPassword($email,$name,$url);
    return new WP_REST_Response(array(
        'message' =>'A message has been sent to this email. It expires in an hour',
        //'url'=>$url,
        //'token'=>$reset_token
        ), 200);
}
...
{
    $email = $data['email'];
    $token_from_url = $data['token'];
    $new_password = $data['new_password'];
    $confirm_password = $data['confirm_password'];

    $required_parameters = ['email','token','new_password','confirm_password'];
    $errors = validated($required_parameters, $data);
    if($errors):
        $response = new WP_REST_Response($errors);
        $response->set_status(401);
    ...
    $user = get_user_by('email',$email);
    if (!$user)
        return new WP_Error('user_not_found', 'invalide email', array('status' => 404));

    $storage_token = get_field('token_reset_password','user_'.$user->ID);
    //$storage_token = $storage_token ? json_decode($storage_token,true) : [];
    $token_storage_for_this_user = $storage_token['token'];
    $expire_at_storage_for_this_user = $storage_token['expire_at'];

    if($token_from_url !== $token_storage_for_this_user || time() >$expire_at_storage_for_this_user)
        return new WP_Error('invalid_token', 'Invalid or expired token', array('status' => 400));

    if($new_password !== $confirm_password)
        return new WP_Error('password_not_match', 'password and confirm password not match', array('status' => 400));
    wp_set_password($new_password, $user->ID);
    return new WP_REST_Response( array(
        'message'=>'password changed successfully !!!',
//        'time' =>time(), // 1738099317
//        'token_url' =>$token_from_url,
//        'expire_at' =>$storage_token['expire_at'],
